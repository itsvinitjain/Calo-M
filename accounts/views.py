...
def main(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            fat_target = int(target.daily_fat) if target else 0
            protein_target = int(target.daily_protein) if target else 0
            carbs_target = int(target.daily_carbohydrates) if target else 0
            if fat_target > 1:
